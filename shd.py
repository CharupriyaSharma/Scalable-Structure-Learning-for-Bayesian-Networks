from main import readBif, readBNfile
import sys

def compute_shd(biffile, BNfile):
    bifmap = readBif(biffile)
    ...
    for child, v in bifmap.items():
        ...
        flipped_edges += len([parent for parent in gobnilp - ground_truth if parent in bifmap.keys() and child in bifmap[parent]])
    ...
    tp = correct_edges + flipped_edges
    fpn = missing_edges + additional_edges

    f1score = (float (tp)) / (float(tp) + 0.5*fpn)
    #return [f1score, tp, fpn] 
    print([correct_edges, flipped_edges, missing_edges, additional_edges])
